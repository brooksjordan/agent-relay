# Lessons Learned: Debugging the Agent Relay Loop

**Date:** 2026-01-30
**Project:** Agent Relay automation
...
Claude Code resolves relative paths from its current working directory. If you run `loop.ps1` from the scripts directory, Claude will try to create `src/components/Foo.tsx` relative to that scripts directory instead of the project root.
...
```powershell
Start-Process powershell -ArgumentList '-NoExit', '-Command', 'cd C:/agent-relay/scripts; ./overnight.ps1 -ProjectPath C:/your-project -ProjectName "Your Project" -MaxIterations 30'
```
...
## 2026-02-01 — Always Launch Agent Relay in a Visible Window

### Requirement

When running Agent Relay from Claude Code or any automated context, **always launch it in a separate visible PowerShell window** so the user can monitor progress in real-time.
...
Start-Process powershell -ArgumentList @(
    ...
    '-NoExit'
    '-Command', "cd agent-relay\scripts; .\auto-compound.ps1 -ProjectPath 'C:\your-project' -Verbose"
)
```
...
Add to any Claude Code instructions or CLAUDE.md files that use Agent Relay:
```
When running Agent Relay, always launch in a separate visible window using Start-Process powershell with -NoExit flag.
```
...
## 2026-02-20 -- Node.js Project Compatibility (5 Critical Fixes)

### Context

The pipeline (graded A+ by Deep Think after 3 rounds of review) was built and hardened on a stdlib-only Python project with zero external dependencies. The first deployment to a Node.js/TypeScript project (Next.js 16 + Tailwind CSS 4 + Recharts 3) exposed 5 distinct failures across 3 pipeline launches.

All 5 shared a common theme: **the pipeline assumed a clean, warning-free git and build environment that Node.js projects don't provide.**

Analysis: `logs/pipeline-nodejs-issues-analysis.md`
Tribunal: Codex B+ / Gemini C+
Deep Think referee: A- (adjudicated all 5 issues + tribunal concerns)

### Issue 1: `git stash --all` chokes on node_modules

**Location:** `auto-compound.ps1` Stage 0
**Severity:** Critical (pipeline hangs at Stage 0)

Deep Think Review #2 had correctly changed `--include-untracked` to `--all` to capture gitignored files (.env, SQLite). But `--all` also captures `node_modules/` (100K+ files), `.venv/`, and other massive gitignored dependency directories. Git tries to stash all of them, emitting thousands of LF/CRLF warnings for 10+ minutes.

**Fix:** Change `git stash --all` to `git stash push --include-untracked`. This captures tracked + untracked files but **leaves gitignored files alone**, letting `.gitignore` do the work. No hardcoded directory deletion lists needed.

**Deep Think insight:** Both the initial analysis (pre-delete directories) and Codex's tribunal suggestion (pathspec exclusion) were over-engineered. The native `--include-untracked` flag is the complete solution.

### Issue 2: Quality check stderr warnings treated as terminating errors

**Location:** `loop.ps1` `Invoke-QualityChecks` function
**Severity:** Critical (tasks fail despite passing)

With `$ErrorActionPreference = "Stop"`, `Invoke-Expression $check 2>&1` converts stderr output from build tools into PowerShell ErrorRecord objects. These trigger the catch block even when the command succeeds (exit code 0).

Observed: `npm install --prefer-offline` emits `EBADENGINE` warnings to stderr (Node.js version compatibility notice). Command succeeds, but quality check reports failure.

**Fix:** Created `Invoke-SafeExpression` in new `common.ps1` that temporarily lowers `$ErrorActionPreference` to `"Continue"` and relies on exit codes.

### Issue 3: Orchestrator git commit stderr warnings treated as terminating errors

**Location:** `loop.ps1` orchestrator commit block
**Severity:** Critical (verified code cannot be committed)

Same root cause as Issue 2. `& git add . 2>&1 | Out-Null` with `$ErrorActionPreference = "Stop"` converts git CRLF warnings to terminating exceptions. Deep Think Patch 1 (orchestrator failure differentiation) correctly preserved the files, but the task was still marked failed.

**Fix:** Replace bare `& git` calls with `Invoke-Native git -C $WorkspaceRoot` which safely handles stderr via `$ErrorActionPreference = "Continue"`.

### Issue 4: `git clean -fd` deletes loop.log's parent directory

**Location:** `loop.ps1` dirty tree cascade
**Severity:** Medium (pipeline crashes on path-not-found after task failure)

The log file defaulted to `tasks/loop.log`. When dirty tree cascade runs `git clean -fd`, it deletes untracked directories including `tasks/`. The next `Write-Log` call fails with "Could not find a part of the path."

This never happened on the Python project because `tasks/` was committed to the feature branch. On the Node.js project, the tasks directory was untracked because the first task failed before the orchestrator could commit.

**Fix:** Move the log file to `$PSScriptRoot\..\logs\` (inside `agent-relay/logs/`, outside the workspace). This makes it permanently immune to `git clean -fd`.

**Deep Think insight:** "The orchestrator's state and logs belong to the orchestrator, not the target repository."

### Issue 5: Pipeline continues through Stages 7-9 with 0 tasks completed

**Location:** `auto-compound.ps1` Stage 8 gate
**Severity:** High (state file deleted, preventing resume)

When the loop aborted early (orchestrator failure), the result had `completed=0, failed=null, pending=14`. In PowerShell, `[int]$null` evaluates to `0`, so `failed -gt 0` was false. The gate didn't fire. The pipeline proceeded to create an empty PR (failed), try to mark priority complete (failed), run post-mortem, and delete the state file -- destroying the ability to resume.

**Fixes:**
1. Wrap `Where-Object` results in `@(...)` to guarantee integer counts (prevents `$null.Count` ambiguity)
2. Change Stage 8 gate from `failed -gt 0` to `completed -lt totalTasks` (catches both failed AND pending tasks)
3. Add `exit 1` when gate fires to preserve state file (stops pipeline instead of continuing)
4. Add zero-completed gate before Stage 7 to skip push/PR entirely when nothing was built

### Root Cause Pattern

Issues 2 and 3 share the same root cause: **PowerShell's `$ErrorActionPreference = "Stop"` converts stderr output to terminating exceptions.** Every external tool that emits warnings to stderr (git, npm, pip, gh, cargo, tsc) is vulnerable.

### Solution: `common.ps1` Shared Module

Created `scripts/common.ps1` with two safe wrappers, dot-sourced by both `auto-compound.ps1` and `loop.ps1`:

- `Invoke-Native` -- for `& command args` calls (git, gh, npm)
- `Invoke-SafeExpression` -- for `Invoke-Expression $string` calls (quality checks, verify commands)

Both temporarily lower `$ErrorActionPreference` to `"Continue"` and return a PSCustomObject with `.ExitCode` and `.Output`. All 10 bare external process calls in loop.ps1 were wrapped.

### Files Changed

| File | Changes |
|------|---------|
| `scripts/common.ps1` | **NEW** -- Invoke-Native + Invoke-SafeExpression (54 lines) |
| `scripts/auto-compound.ps1` | Dot-source common.ps1, remove local Invoke-Native, fix stash, add gates |
| `scripts/loop.ps1` | Dot-source common.ps1, move log file, wrap all 10 external calls, fix counts |

### Key Takeaways

1. **Let `.gitignore` do the work** -- `git stash push -u` skips gitignored files automatically. Don't maintain deletion lists.
2. **Trust exit codes, never stderr** -- Every external command must go through Invoke-Native or Invoke-SafeExpression.
3. **Orchestrator logs belong outside the workspace** -- anything inside the project tree is vulnerable to `git clean`.
4. **Gate on completion, not failure** -- `completed < total` catches both failed and pending. `failed > 0` misses the zero-completed case.
5. **`[int]$null` is 0 in PowerShell** -- wrap `Where-Object` results in `@(...)` to guarantee integer counts.
6. **Test on a real ecosystem project before declaring A+** -- stdlib-only Python hides an entire class of stderr/dependency/gitignore issues.

*Diagnosed 2026-02-20. Tribunal: Codex B+ / Gemini C+. Deep Think referee: A-. Fixed 2026-02-21.*
