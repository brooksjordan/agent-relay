...
# Test manually
Start-ScheduledTask -TaskName "AgentRelay-CompoundReview"
...
|------|------|---------|----------|
| `AgentRelay-KeepAwake` | 10:00 PM | Prevent sleep | 4 hours |
| `AgentRelay-CompoundReview` | 10:30 PM | Extract learnings | ~30 min |
| `AgentRelay-AutoCompound` | 11:00 PM | Implement + PR | ~1-2 hours |
...
| `-TaskPrefix` | `AgentRelay` | Prefix for task names |
...
Get-ScheduledTask -TaskName "AgentRelay*" | Format-Table TaskName, State, LastRunTime
```
...
The `AgentRelay-KeepAwake` task uses Windows API (`SetThreadExecutionState`) to:
...
4. **Verify WakeToRun is enabled:**
   ```powershell
   Get-ScheduledTask -TaskName "AgentRelay*" |
...
1. Check task state:
   ```powershell
   Get-ScheduledTask -TaskName "AgentRelay-CompoundReview"
   ```

2. If disabled, enable it:
   ```powershell
   Enable-ScheduledTask -TaskName "AgentRelay-CompoundReview"
   ```
