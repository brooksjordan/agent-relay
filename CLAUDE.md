# CLAUDE.md — Agent Relay

This file provides guidance to Claude Code when working in the agent-relay repository.

## Purpose

Agent Relay is an autonomous overnight development system using Claude Code CLI. It executes a priority backlog while the user sleeps, producing PRs by morning.
...
```powershell
.\scripts\launch-auto-compound.ps1 -ProjectPath "./your-project" -Verbose -DryRun
```
...
│   ├── common.ps1            # Shared utilities: Invoke-Native, Invoke-SafeExpression
...
└── docs/
    ...
    ├── lessons-learned.md    # Critical debugging knowledge (READ THIS)
    └── ...
```

## Critical Knowledge

**Before modifying any script, read `docs/lessons-learned.md`.** It contains hard-won debugging lessons that prevent regression of fixed bugs.
...
|-------|------|-------------|
| 0 | **Preflight** -- safety stash (`push -u`), self-healing cleanup, check for uncommitted work, report on remote, open priorities | No (safe stash + cleanup) |
...
| 4v | Validate PRD (critic + optional regen) | No |
...
| 2026-02-21 | `common.ps1` shared module | Invoke-Native + Invoke-SafeExpression dot-sourced by both scripts |
| 2026-02-21 | Stash fix: `--all` to `push -u` | Prevents choking on node_modules (100K+ gitignored files) |
| 2026-02-21 | Wrap all external calls in loop.ps1 | 10 bare git/npm calls wrapped with Invoke-Native/SafeExpression |
| 2026-02-21 | Log file evacuated to agent-relay/logs | Immune to `git clean -fd` in workspace |
| 2026-02-21 | Stage 8 gate: `completed < total` | Catches both failed AND pending; `exit 1` preserves state file |
| 2026-02-21 | Zero-completed gate before Stage 7 | Skips push/PR/mark when nothing was built |
...
4. Document in lessons-learned.md if you discover non-obvious behavior
...
This project follows the Computronium methodology:
...
**Status:** Working. Node.js compatibility hardened. Shared `common.ps1` module. Stages 0-9 with resume, PRD validation, replan, and post-mortem.
